...
distances.evol.lmer.4 <- update(distances.evol.lmer.2, . ~ . - step:dist_type:theory) # 2. Adding step:dist_type:memory_type didn't improve the model
#distances.evol.lmer.5 <- update(distances.evol.lmer.4, . ~ . - step:dist_type:memory_type)
distances.evol.lmer.6 <- update(distances.evol.lmer.4, . ~ . - memory_type:theory)
distances.evol.lmer.7 <- update(distances.evol.lmer.6, . ~ . - dist_type:theory)
distances.evol.lmer.8 <- update(distances.evol.lmer.7, . ~ . - dist_type:memory_type) # 1. Adding step:theory didn't improve the model
#distances.evol.lmer.9 <- update(distances.evol.lmer.8, . ~ . - step:theory)
distances.evol.lmer.10 <- update(distances.evol.lmer.8, . ~ . - step:memory_type)
distances.evol.lmer.11 <- update(distances.evol.lmer.10, . ~ . - step:dist_type)
distances.evol.lmer.12 <- update(distances.evol.lmer.11, . ~ . - theory)
distances.evol.lmer.13 <- update(distances.evol.lmer.12, . ~ . - memory_type)
distances.evol.lmer.14 <- update(distances.evol.lmer.13, . ~ . - dist_type)
distances.evol.lmer.15 <- update(distances.evol.lmer.14, . ~ . - step)
...
distances.evol.fixed <- distances.evol.lmer.0

# GLOBAL MODEL -- RANDOM EFFECTS
distances.evol.random.0 <- update(distances.evol.fixed, . ~ . - (step | subject))
distances.evol.random.1 <- update(distances.evol.random.0, . ~ . + (1 + step | subject)) # Adding random slope for step didn't improve the model
#distances.evol.random.2 <- update(distances.evol.random.0, . ~ . + (1 + step + dist_type | subject))
# Didn't converge
#distances.evol.random.3 <- update(distances.evol.random.0, . ~ . + (1 + step + dist_type + memory_type | subject))
# Didn't converge
# Comparing random effects models
distances.evol.random.comparison <- anova(distances.evol.random.0,
                                          distances.evol.random.1)
#print(distances.evol.random.comparison)
# Select final model
distances.evol.final <- distances.evol.random.0
